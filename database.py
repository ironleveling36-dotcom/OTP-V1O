...
def _run_migrations():
    ...
    # ── Used transactions (prevents UTR reuse for auto-verified recharges) ──
    _execute("""
...
def get_purchase_otps(purchase_id):
    return _execute(
        ...
        (purchase_id,), "all",
    )# ── Used transactions (UTR reuse prevention for auto-verified recharges) ──────

def is_txn_used(txn_id: str) -> bool:
    """Check if a transaction ID has already been used for an auto-verified recharge."""
    row = _execute(
        "SELECT 1 FROM used_transactions WHERE txn_id = ?", (txn_id,), "one"
    )
    return row is not None


def mark_txn_used(txn_id: str, user_id: int, username: str, amount) -> None:
    """Mark a transaction ID as used (prevents double-claiming the same UTR)."""
    import time
    _execute(
        "INSERT OR IGNORE INTO used_transactions "
        "(txn_id, user_id, username, amount, approved_at) VALUES (?, ?, ?, ?, ?)",
        (txn_id, user_id, username, amount, int(time.time())),
    )