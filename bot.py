...
async def text_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if ctx.user_data.get("awaiting_txn_id"):
        ...
        # ── Duplicate-UTR guard (runs ALWAYS, even in manual mode) ──────────
        # A transaction ID can only ever be used once across the whole bot,
        # whether it was auto-verified OR manually approved by an admin.
        if db.is_txn_used(txn_id):
            await update.message.reply_text(
            ...
            db.add_log("recharge", f"Duplicate UTR {txn_id} rejected for req #{req_id}", user_id)
            for admin_id in config.ADMIN_IDS:
                try:
                    await ctx.bot.send_message(
                        chat_id=admin_id,
                        text=(
                            f"🔁 *Reused UTR Attempt Blocked*\n\n"
        ...
        # ── Auto-verification via Gmail IMAP ──────────────────────────────
        # If Gmail is configured, try to find the transaction ID in recent
        # payment-alert emails. If found and the amount matches, auto-approve
        # the recharge instantly — no admin action needed.
        # If Gmail is not configured, or the txn is not found, or the amount
        # doesn't match, fall back to manual admin approval.
        ...
        if config.GMAIL_ADDRESS and config.GMAIL_APP_PASSWORD:
            try:
                result = gmail_checker.find_transaction(txn_id)
            except Exception as e:
                logger.warning("Gmail check error for txn %s: %s", txn_id, e)
                result = {"found": False, "amount": None, "error": str(e)}

            auto_error = result.get("error")

            if result["found"]:
                # Validate amount — accept if email amount >= requested amount
                email_amount = result.get("amount")
                if email_amount is not None and email_amount >= amount:
                    # ✅ Auto-approve!
                    db.update_recharge_request(req_id, "approved")
                    db.mark_txn_used(txn_id, user_id, uname or "", email_amount)
                    new_bal = db.credit_wallet(user_id, amount, "Wallet Recharge (Auto-Verified)")
                    db.add_log(
                        "recharge",
                        f"Auto-approved recharge #{req_id}: +₹{amount:.2f} "
                        f"to user {user_id} (txn {txn_id}, email amount ₹{email_amount:.2f})",
                        user_id,
                    )
                    ...
                    await status_msg.edit_text(
                        ...
                        f"🧾 Transaction ID: `{txn_id}`\n"
                        f"🆔 Request ID: `{req_id}`\n\n"
                        f"Your wallet has been credited instantly! 🎉\n"
                    ...
                    for admin_id in config.ADMIN_IDS:
                        try:
                            await ctx.bot.send_message(
                                chat_id=admin_id,
                                text=(
                                    f"🤖 *Auto-Verified Recharge*\n\n"
                                    f"👤 *User:* {uname} (ID: `{user_id}`)\n"
                                    f"💰 *Amount:* `₹{amount:.2f}`\n"
                                    f"🧾 *Transaction ID:* `{txn_id}`\n"
                                    f"📧 *Email Amount:* `₹{email_amount:.2f}`\n"
                                    f"🆔 *Request ID:* `{req_id}`\n\n"
                                    f"✅ Auto-approved via Gmail verification."
                                ),
                                parse_mode=ParseMode.MARKDOWN,
                            )
                        except Exception:
                    ...
                    auto_approved = True
                elif email_amount is not None and email_amount < amount:
                    # Amount mismatch — reject and notify
                    await status_msg.edit_text(
                        f"❌ *Amount Mismatch*\n\n"
                        f"Expected: `₹{amount:.2f}`\n"
                        f"Found in email: `₹{email_amount:.2f}`\n\n"
                    ...
                    db.add_log(
                        "recharge",
                        f"Auto-rejected req #{req_id}: amount mismatch "
                        f"(expected ₹{amount:.2f}, email ₹{email_amount:.2f})",
                        user_id,
                    )
                    ...
                    for admin_id in config.ADMIN_IDS:
                        try:
                            await ctx.bot.send_message(
                                chat_id=admin_id,
                                text=(
                                    f"⚠️ *Auto-Verify: Amount Mismatch*\n\n"
                                    f"👤 *User:* {uname} (ID: `{user_id}`)\n"
                                    f"💰 *Expected:* `₹{amount:.2f}`\n"
                                    f"📧 *Email Amount:* `₹{email_amount:.2f}`\n"
                                    f"🧾 *Transaction ID:* `{txn_id}`\n"
                            ...
                            pass
                    auto_approved = True  # handled — don't fall through to manual
                # If email_amount is None, we found the txn but couldn't parse
                # the amount — fall through to manual approval for safety.
            # If not found, fall through to manual approval

        # ── Fallback: Manual admin approval ───────────────────────────────
        if not auto_approved:
            db.update_recharge_request(req_id, "pending")

            if auto_error and auto_error != "gmail_not_configured":
                logger.warning("Gmail auto-verify error for req #%s: %s", req_id, auto_error)
            ...
            await status_msg.edit_text(
                ...
                f"🧾 Transaction ID: `{txn_id}`\n"
                f"🆔 Request ID: `{req_id}`\n\n"
                f"Your payment has been sent for verification. "
                f"Your wallet will be credited *after approval*.\n"
                f"_(Bank emails can take 1–2 minutes. If not auto-verified, "
                f"an admin will review it manually.)_",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=main_menu_keyboard()
            )
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── Admin recharge approvals/rejections ─────────────────────
        ...
        # Block manual approval of an already-used UTR (defence in depth)
        _txn = req.get("txn_id")
        if _txn and db.is_txn_used(_txn):
            db.update_recharge_request(req_id, "rejected")
            await query.answer("⚠️ This Transaction ID was already used. Approval blocked.", show_alert=True)
            await query.edit_message_text(
                f"⚠️ *Approval Blocked — Duplicate UTR*\n\n"
                f"Transaction ID `{_txn}` has already been credited once.\n"
                f"Request `{req_id}` has been rejected to prevent a double credit.",
                parse_mode=ParseMode.MARKDOWN,
            )
            db.add_log("admin", f"Blocked duplicate UTR {_txn} on manual approve #{req_id}", user_id)
            return

        db.update_recharge_request(req_id, "approved")
        new_bal = db.credit_wallet(req["user_id"], req["amount"], "Wallet Recharge Approved")
        # Record the UTR so it can never be reused (auto OR manual)
        if _txn:
            db.mark_txn_used(_txn, req["user_id"], req.get("username") or "", req["amount"])
