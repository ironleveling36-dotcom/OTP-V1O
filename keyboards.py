...
_CB_DATA_LIMIT = 64  # Telegram callback_data hard limit (BYTES, not chars)

def _safe_cb(data: str) -> str:
    """
    Truncate callback_data to Telegram's 64-byte limit WITHOUT splitting
    multi-byte UTF-8 characters.

    BUG FIX: previous version did `data.encode()[:64].decode(errors="ignore")`
    which could split a multi-byte emoji/character at byte 64, producing
    invalid UTF-8 that was silently dropped — sometimes leaving only a few
    bytes and breaking callback matching.

    New approach: encode, then walk back from byte 64 to the nearest valid
    UTF-8 boundary. If truncation would lose data, append a short hash so
    callbacks remain unique.
    """
    import hashlib
    encoded = data.encode("utf-8")
    if len(encoded) <= _CB_DATA_LIMIT:
        return data
    # Find the largest valid UTF-8 prefix that fits in 64 bytes.
    # A UTF-8 continuation byte is any byte in 0x80–0xBF. If we cut at a
    # continuation byte, we'd split a multi-byte character. Back up until
    # we're at a character boundary.
    cut = _CB_DATA_LIMIT
    while cut > 0 and 0x80 <= encoded[cut] <= 0xBF:
        cut -= 1
    truncated = encoded[:cut].decode("utf-8", errors="ignore")
    # Append a short hash so truncated callbacks stay unique.
    sig = hashlib.md5(data.encode("utf-8")).hexdigest()[:6]
    return f"{truncated}#{sig}"
